...
router.get('/paciente/:dni', function(req, res){
    var dniPaciente = req.params.dni;
    pacientesDb.getByDni(dniPaciente, function(err, data){
        if (err) return res.status(500).send(err.message);
        if (data.length == 0) return res.status(404).send('El paciente con dni '+dniPaciente+' no se encueentra en el sistema');
        res.json(data);
    });
});

router.get('/paciente/empleado/:cifClinica', function(req, res ){
    var cifClinica = req.params.cifClinica;
    consultasDb.getByCif(cifClinica, function(err, data){
        if (err) return res.status(500).send(err.message);
